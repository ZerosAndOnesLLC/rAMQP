...
> **Status: building — Phases 0–9 complete; Phase 10 substantially done** (in CI/verified: fe2o3 + Qpid JMS + Qpid Proton interop against our broker, the unified conformance harness, in-process fault injection **and** process-level netns split-brain with a concurrent load sweep, the runtime decision, and the tuned-incumbent + durability-parity + Artemis-tuned bench re-run (provisional). Remaining: bare-metal bench numbers, a latency-under-N-connections bench sweep, and — deferred — our-client⇄Qpid Broker-J (image-config quirk, already covered by the RabbitMQ + Artemis interop jobs)). See §11 checkboxes. The broker runs **clustered**:
...
- [~] Publish the full tail-latency/RSS comparison vs tuned incumbents: the published matrix (bench-compare/README) covers transient/quorum/clustered/deep-queue legs vs **default-config** RabbitMQ 4.3.1 and Artemis with honest caveats. **Tuned re-run + durability-parity quorum landed** (`bench-compare/tuned/`: `rabbitmq.conf` + one-command `run.sh`, all legs over loopback TCP for identical transport): transient p50 ≈ 2.8× below *tuned* RabbitMQ classic, and — closing the "in-memory vs fsync" caveat — our `store-redb` fsync quorum p50 ≈ 8× below RabbitMQ's fsync quorum. **Tuned Artemis leg added** too (NIO journal, autotune off): transient p50 ≈ 3× below it. **Numbers are provisional (WSL2, indicative only)**; the defend-forever figures come from quiet bare metal via the same script. Remaining: bare-metal run, and a latency-under-N-concurrent-connections sweep (the table is single-connection closed-loop). README + docs updated per phase.
